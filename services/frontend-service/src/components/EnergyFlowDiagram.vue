          ...
          <line 
            ...
            :class="{
              ...
              [`flow-${connection.direction}`]: 
                ...
                (connection.from !== 'battery' || batteryLevel > 10.01)
            }"
          />
      ...
      <div class="charts-column" v-if="hourlyDataLoaded">
        <div class="environmental-charts">
          ...
          <div class="chart-container" style="margin-top: 20px;">
            <h4>Water Temperature (°C)</h4>
            <div class="chart-wrapper">
              <div id="water-temp-chart-container" style="height: 160px; width: 100%;"></div>
            </div>
            <div class="current-value" v-if="energyModels">
              Current: {{ energyModels.waterTemperature.toFixed(1) }}°C
            </div>
          </div>
...
import { loadConfig, processApplianceConfig, processPowerSourcesConfig, getSimulationSettings } from '../../configLoader.js';
...
import { generateApplianceSchedules } from './otherAppliances.js'
...
export default {
  ...
  data() {
    return {
      ...
      applianceSchedules: null,
      aiScheduleDownloaded: false
    };
  },
  ...
  computed: {
    ...
    energyStats() {
      ...
      // Add EV SoC if connected
      if (this.energyModels && this.energyModels.evConnected) {
        stats.push({
          type: 'ev',
          title: 'EV Charge',
          value: `${Math.round(this.energyModels.evSoC * 100)}%`
        });
      }
  ...
  methods: {
    ...
    initWaterTempChart(container, timeLabels, customLabels) {
      ...
      if (!this.chartData.temperature.water[0] && this.energyModels.waterTemperature) {
    ...
    initPriceChart(container, timeLabels, customLabels) {
      ...
      // Ensure both data arrays exist and are valid arrays
      if (!this.chartData.price.primary) {
        this.chartData.price.primary = Array(96).fill(null);
      }
      
      if (!this.chartData.price.energy) {
        this.chartData.price.energy = Array(96).fill(0);
      }

      // Ensure data arrays are properly defined
      const priceData = [...this.chartData.price.primary];
      const energyData = [...this.chartData.price.energy];
      ...
      const chartConfig = {
        ...
        series: [
          {
            name: 'Energy (kWh)',
            type: 'column',
            data: this.chartData.price.energy
          },
          {
            name: 'Price',
            type: 'line',
        ...
        fill: {
          opacity: [0.6, 1]  // Make bars semi-transparent
        },
        plotOptions: {
          column: {
            borderRadius: 2,
        ...
        yaxis: [
          {
            // Left Y-axis for energy consumption (kWh)
            seriesName: 'Energy (kWh)',
            min: 0,
            max: 3,
            tickAmount: 3,
            title: {
        ...
        tooltip: {
          ...
          y: {
            formatter: (value, { seriesIndex }) => {
              if (seriesIndex === 0) {
                return value === null || value === undefined ? 'N/A' : `${value.toFixed(2)} kWh`;
              } else {
                return value === null || value === undefined ? 'N/A' : `$${value.toFixed(3)}/kWh`;
              }
            }
          }
        },
    ...
    updateChartDataPoint(chartName, step, value, newVal = null, waterTemp = null, energyConsumption = null) {
      ...
      // Price chart handling - now includes energy consumption
      // Update energy consumption data if provided
      if (energyConsumption !== null) {
        if (!this.chartData[chartName].energy) {
          this.chartData[chartName].energy = Array(96).fill(0);
        }
        this.chartData[chartName].energy[step] = energyConsumption;
      }
      
      // Update price chart with both price and energy consumption
      if (this.charts.price) {
        this.charts.price.updateSeries([
          {
            name: 'Energy (kWh)',
            type: 'column',
            data: this.chartData[chartName].energy
          },
          {
            name: 'Price',
            type: 'line',
    ...
    updateCharts(step) {
      ...
      const currentGridPower = this.gridPower;
      ...
      this.updatePriceChart(step, hourData.price, currentGridPower);

      // Update battery history with current value
      this.updateBatteryHistory(step, currentBatteryLevel);

      // Add memory optimization every 20 steps
      if (step % 20 === 0) {
    ...
    updateTemperatureCharts(step, outdoorTemp, indoorTemp, waterTemp) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        // Update air temperature data
        this.chartData.temperature.primary[stepIndex] = outdoorTemp;
        if (indoorTemp !== null && indoorTemp !== undefined) {
          this.chartData.temperature.indoor[stepIndex] = indoorTemp;
        }

        // Specifically update water temperature data
        if (waterTemp !== null && waterTemp !== undefined) {
          this.chartData.temperature.water[stepIndex] = waterTemp;
        }
      }
    ...
    updatePriceChart(step, priceValue, gridPower = 0) {
      const hourStart = Math.floor(step / 4) * 4;
      
      // Calculate energy consumption (kWh) for this time interval
      // gridPower (kW) * 0.25 hours (15 minutes) = energy (kWh)
      const energyConsumption = Math.max(0, gridPower * 0.25);
      
      // Update the energy data for the current step only
      // This keeps the data clean and targeted to actual simulation points
      if (!this.chartData.price.energy) {
        this.chartData.price.energy = Array(96).fill(0);
      }
      this.chartData.price.energy[step] = energyConsumption;
      
      // Update price data for the same step
      if (!this.chartData.price.primary) {
        this.chartData.price.primary = Array(96).fill(null);
      }
      this.chartData.price.primary[step] = priceValue;
      
      // Update the chart directly with the full arrays
      if (this.charts.price) {
        this.charts.price.updateSeries([
          {
            name: 'Energy (kWh)',
    ...
    optimizeChartPerformance() {
      // Reduce data points in history for better performance
      if (this.simulationSteps > 95) {
        // Only keep recent and future data points
        const currentStep = this.simulationSteps % 96;
        const keepFrom = Math.max(0, currentStep - 20);
        const keepTo = Math.min(95, currentStep + 20);
        
        // For steps outside the view window, use simplified data
        for (let i = 0; i < 96; i++) {
          if (i < keepFrom || i > keepTo) {
            // Keep only every 4th point outside viewing window
            if (i % 4 !== 0) {
              if (this.chartData.price.energy) {
                this.chartData.price.energy[i] = 0;
              }
            }
          }
        }
      ...
      if (this.charts.price) {
        this.charts.price.updateOptions({
          chart: {
            animations: {
              enabled: false
            }
    ...
    async loadDefaultConfig() {
      try {
        const config = await loadConfig('/appliances-config.json');
    ...
    initializePowerSources(sourcesConfig) {
      this.powerSources = [
        ...
        },
        {
          ...
          type: 'battery',
          capacity: sourcesConfig.battery?.capacity || 6.4,
    ...
    toggleApplianceStatus(id) {
      ...
      if (appIndex !== -1) {
        const currentlyActive = this.simulatorAppliances[appIndex].active;
    ...
    updateHistoryData() {
      ...
      console.log("==== Appliance Scheduling Logic ====");
      console.log("Washer Status:", this.rlPrediction.appliances.shiftable.wash_machine);
      console.log("Dryer Status:", this.rlPrediction.appliances.shiftable.clothes_dryer);
  
      // Check if washing machine has completed
      const washingComplete = this.rlPrediction.appliances.shiftable.wash_machine.progress >= 
                               this.rlPrediction.appliances.shiftable.wash_machine.total_duration;
      
      console.log("Washing Complete:", washingComplete);
      console.log("Current Time Step:", this.rlPrediction.timestamp);
      ...
      try {
        ...
        if (timeStep >= 0 && timeStep < 96) {
          ...
          if (this.rlPrediction.appliances && this.rlPrediction.appliances.shiftable) {
            const newAppliancePowerHistory = JSON.parse(JSON.stringify(this.appliancePowerHistory));

            // Define required durations for each appliance
            const applianceDurations = {
              'dishwasher': 3,      // K_DishWasher = 3
              'wash_machine': 6,    // K_WashMachine = 6
              'clothes_dryer': 5     // K_ClothesDryer = 5
            };

            // Define standard power values for each appliance
            const applianceStandardPowers = {
              'dishwasher': 1.8,      // Default power for dishwasher
              'wash_machine': 0.4,    // Default power for washing machine
              'clothes_dryer': 1.2     // Default power for clothes dryer
            };
            
            // Store information about when appliances were started
            if (!this.applianceStartTimes) {
              this.applianceStartTimes = {
                'dishwasher': -1,
                'wash_machine': -1,
                'clothes_dryer': -1
            ...
            const washMachine = this.rlPrediction.appliances.shiftable.wash_machine;
            const washingCompleted = washMachine && (washMachine.progress >= washMachine.total_duration);
            ...
            for (const appKey in this.rlPrediction.appliances.shiftable) {
              ...
              if (app) {
                const duration = applianceDurations[appKey];
                let power = 0;

                // If the appliance is currently active according to RL and hasn't been started yet
                if (app.active && this.applianceStartTimes[appKey] === -1) {
                  // Record when this appliance was first started
                  if (appKey !== 'clothes_dryer') {
                    this.applianceStartTimes[appKey] = timeStep;
                  } else {
                    if (washingCompleted) {
                      this.applianceStartTimes[appKey] = timeStep;
                    }
                ...
                // If this appliance has been started
                if (this.applianceStartTimes[appKey] !== -1) {
                  // Calculate elapsed steps since it was started
                  const stepsElapsed = timeStep - this.applianceStartTimes[appKey];
                  
                  // If we're within the required duration, turn it on regardless of RL's current decision
                  if (stepsElapsed >= 0 && stepsElapsed < duration) {                
                    power = applianceStandardPowers[appKey];
                    newAppliancePowerHistory[appKey][timeStep] = power;
                  } else {
                    newAppliancePowerHistory[appKey][timeStep] = 0;
                  }

                } else {
                  // Appliance hasn't been started yet, use RL's decision
                  newAppliancePowerHistory[appKey][timeStep] = 0;
                }
              }
            }
    ...
    // Add this method to the methods section in EnergyFlowDiagram.vue
    runHeadlessSimulation() {
      console.log("Starting headless simulation for data recording...");
      
      // Step 1: Capture current state to use as initial conditions
      const initialState = {
        // Environment data
        day: this.localSimulationDay,
        startTime: this.simulationTime,
        solarOutput: this.solarOutput,
        outdoorTemperature: this.currentHourData?.temperature || -99,
        electricityPrice: this.currentHourData?.price || -99,
        ...
        // Appliance states (deep copy to avoid reference issues)
        appliances: JSON.parse(JSON.stringify(this.simulatorAppliances)),
        
        // Current hourly data
        hourlyData: [...this.hourlyData]
      };
      
      // Step 2: Create data container for all 96 timesteps
      const simulationData = [];
      
      // Step 3: Run the simulation for 96 steps (one full day)
      let currentState = {...initialState};

      // Battery charging/discharging logic (matching index.js implementation)
      const BATTERY_MIN_LEVEL = 10.0;
      const MAX_BATTERY_CHARGE_RATE = 2.4; // kW
      const MAX_BATTERY_DISCHARGE_RATE = 2.4; // kW
      
      for (let step = 0; step < 96; step++) {
        // Get the hourly data for current step
        const hourIndex = Math.floor(step / 4) % initialState.hourlyData.length;
        const hourData = initialState.hourlyData[hourIndex];
        
        if (!hourData) {
          console.error(`No hourly data available for step ${step}`);
          continue;
        }
        
        // Calculate various model updates using the same logic as the real simulation
        // HVAC model
        const hvacAppliance = currentState.appliances.find(app => app.type === 'hvac');
        const hvacResult = calculateHvacPower(
          currentState.indoorTemperature,
          hourData.temperature,
          hvacAppliance ? hvacAppliance.active : false
        );
        
        // Water heater model
        const waterHeaterAppliance = currentState.appliances.find(app => app.type === 'water_heater');
        ...
        // EV charging model
        const evAppliance = currentState.appliances.find(app => app.type === 'ev_charger');
        const evConnected = isEvConnected(hourData.hour);
        ...
        // Update appliance states based on schedules
        if (this.applianceSchedules) {
          Object.entries(this.applianceSchedules).forEach(([appType, schedule]) => {
            // Skip Group 2 appliances - they're handled by models above
            if (appType === 'hvac' || appType === 'water_heater' || appType === 'ev_charger') {
              return;
            }
            
            // Special case for refrigerator - always stays on
            if (appType === 'refrigerator') {
              const refrigeratorApp = currentState.appliances.find(app => app.type === 'refrigerator');
              if (refrigeratorApp) {
                refrigeratorApp.active = true;
              }
              return;
            }
            
            // Find the appliance in appliances by type
            const appliance = currentState.appliances.find(app => app.type === appType);
            if (!appliance) return;
            
            // Check if the appliance should be active at this time step
            const shouldBeActive = step >= schedule.startTime && 
                                step < schedule.endTime;
            
            // Update appliance state
            appliance.active = shouldBeActive;
          });
        }
        
        // Calculate various aggregated values
        const houseDemand = currentState.appliances
          .filter(app => app.active)
          .reduce((sum, app) => sum + (parseFloat(app.power) || 0), 0);
        
        // Calculate power flows using similar logic as updateConnectionsStatus
        const solarOutput = hourData.solar;
        let batteryPower = 0;
        let batteryStatus = 'empty';
        let batteryAction = 'none';
        let gridToSolar = 0;
        let gridPower = 0;

        // First handle battery status change when at minimum level
        if (currentState.batteryLevel <= BATTERY_MIN_LEVEL && 
           (currentState.batteryStatus === 'discharging' || currentState.batteryPower < 0)) {
          batteryStatus = 'empty';
          batteryPower = 0;
        }

        // Check if we're in a preferred charging time window (12pm-3pm or 12am-5am)
        const isPreferredChargingTime = (hourIndex >= 12 && hourIndex <= 15) || (hourIndex >= 0 && hourIndex <= 5);
        
        // Simple power flow logic (can be refined to match your exact model)
        ...
        if (isPreferredChargingTime) {
          ...
          if (excessSolar > 0 && currentState.batteryLevel < 100) {
            ...
            if (remainingSolar > 0) {
              ...
              gridPower -= gridToSolar;
            } else {
              gridPower = 0;
            }
          } 
          // If no excess solar but still preferred charging time, charge from grid during night hours
          else if (hourIndex >= 0 && hourIndex <= 5 && currentState.batteryLevel < 100) {
            const trickleCharge = currentState.batteryLevel < 20 ? MAX_BATTERY_CHARGE_RATE : 1.0; // Higher charge rate if very low
            batteryAction = 'charge';
            batteryStatus = 'charging';
            batteryPower = trickleCharge;
            gridPower = remainingDemand + batteryPower; // Grid powers both house and battery
          }
          // If battery is full and we have excess solar, send it directly to grid
          else if (currentState.batteryLevel >= 99 && excessSolar > 0) {
            batteryAction = 'none';
            batteryStatus = 'empty';
            batteryPower = 0;
            gridToSolar = excessSolar;
            gridPower = remainingDemand - gridToSolar; // Negative indicates exporting to grid
          }
          ...
          if (remainingDemand > 0 && currentState.batteryLevel > BATTERY_MIN_LEVEL) {
            ...
            gridPower = Math.max(0, remainingDemand + batteryPower); // batteryPower is negative
          }
          // If excess solar available, charge battery (lower priority than discharging during peak)
          else if (excessSolar > 0 && currentState.batteryLevel < 100 && batteryAction === 'none') {
            batteryAction = 'charge';
            batteryStatus = 'charging';
            batteryPower = Math.min(excessSolar, MAX_BATTERY_CHARGE_RATE);
            // If there's still excess solar after charging battery, send it to grid
            ...
            if (remainingSolar > 0) {
              gridToSolar = remainingSolar;
              // Negative grid draw indicates exporting to grid
              gridPower = remainingDemand - gridToSolar;
            } else {
              gridPower = remainingDemand; // Grid handles any remaining demand
            }
          }
        ...
        const batteryCharge = batteryPower * 0.25 * 0.95; //(15 minutes; efficiency)
        const newBatteryLevel = Math.max(
          BATTERY_MIN_LEVEL, 
          Math.min(
            100, 
            currentState.batteryLevel + (batteryCharge / 6.4 * 100)  // 6.4kWh capacity of battery
          )
        );
        ...
        currentState = {
          ...currentState,
          indoorTemperature: hvacResult.nextIndoorTemp,
          waterTemperature: waterHeaterResult.nextWaterTemp,
          evSoC: evConnected ? evResult.nextSoC : currentState.evSoC,
        ...
        // Format appliance status for data recording
        const appStatus = {};
        currentState.appliances.forEach(app => {
          appStatus[app.type] = app.active ? 1 : 0;
        });
        
        // Record the data for this step
        simulationData.push({
          TimeStep: step,
          SolarPower: solarOutput,
          GridPower: gridPower,
          ElectricityPrice: hourData.price,
          BatterySOC: newBatteryLevel,
          evSOC: evConnected ? evResult.nextSoC : currentState.evSoC,
          IndoorTemperature: hvacResult.nextIndoorTemp,
          OutdoorTemperature: hourData.temperature,
          WaterTemperature: waterHeaterResult.nextWaterTemp,
          HVACPower: hvacAppliance && hvacAppliance.active ? hvacResult.power : 0,
          WaterHeaterPower: waterHeaterAppliance && waterHeaterAppliance.active ? waterHeaterResult.power : 0,
          EVChargerPower: evAppliance && evAppliance.active ? evResult.power : 0,
          DishwasherActive: appStatus.dishwasher || 0,
          WashMachineActive: appStatus.wash_machine || 0,
          ClothesDryerActive: appStatus.clothes_dryer || 0,
          TVActive: appStatus.tv || 0,
          RefrigeratorActive: appStatus.refrigerator || 0,
          LightsActive: appStatus.lights || 0,
          VacuumActive: appStatus.vacuum || 0,
          HairDryerActive: appStatus.hair_dryer || 0
        });
        
        // Log progress occasionally
        if (step % 10 === 0) {
          console.log(`Headless simulation progress: ${step}/96 steps completed`);
        }
      }
      
      // Step 4: Export the simulation data to CSV
      this.exportHeadlessSimulationData(simulationData);
      
      console.log("Headless simulation completed and data exported.");
    },

    exportHeadlessSimulationData(simulationData) {
      ...
      // Create CSV rows
      const rows = simulationData.map(data => {
        return headers.map(key => data[key]).join(',');
      });
      ...
      link.setAttribute('download', `simulation_data_headless_day${this.localSimulationDay}.csv`);
    ...
    initializeEnergyModels(hourData) {
      ...
      // Use setTimeout to ensure it doesn't block the UI initialization
      setTimeout(() => {
        this.runHeadlessSimulation();
      }, 1000); 
    },
    ...
    updateEnergyModels(hourData) {
      ...
      if (waterHeaterAppliance) {
        // Get current water temperature and comfort threshold
        const currentWaterTemp = this.energyModels.waterTemperature;
        const targetTemp = this.waterHeaterConstants.setPoint; // Water heater target temperature
        const minTemp = targetTemp - this.waterHeaterConstants.comfortRange; // Turn on when 3°C below target
        const shouldBeActive = currentWaterTemp < minTemp;

        // Calculate power with current state
        const waterHeaterResult = calculateWaterHeaterPower(
          currentWaterTemp,
          waterHeaterAppliance.active,
          hourData.hour
        );
        
        // Update water temperature based on model
        this.energyModels.waterTemperature = waterHeaterResult.nextWaterTemp;

        // waterHeaterAppliance.power = parseFloat(waterHeaterResult.power.toFixed(1));
        this.updateAppliancePower(waterHeaterAppliance.id, waterHeaterResult.power);

        if (shouldBeActive && !waterHeaterAppliance.active) {
          this.toggleApplianceStatus(waterHeaterAppliance.id);
        } else if (!shouldBeActive && waterHeaterAppliance.active && currentWaterTemp >= targetTemp) {
          this.toggleApplianceStatus(waterHeaterAppliance.id);
        }
      ...
      if (evAppliance) {
        // Update EV connection status based on time of day
        const previousConnected = this.energyModels.evConnected;
        this.energyModels.evConnected = isEvConnected(hourData.hour);

        // If EV connection status changed
        if (previousConnected !== this.energyModels.evConnected) {
          if (this.energyModels.evConnected) {
            // EV just connected
            const shouldCharge = hourData.price < 0.2 && this.energyModels.evSoC < 0.8;
            
            if (shouldCharge && !evAppliance.active) {
              this.toggleApplianceStatus(evAppliance.id);
            }
          } else {            
            // Turn off charger if it was on
            if (evAppliance.active) {
              this.toggleApplianceStatus(evAppliance.id);
            }
          }
        }
        
        // Calculate EV charging parameters
        const evResult = calculateEvChargingPower(
          this.energyModels.evSoC,
          this.energyModels.evConnected,
          evAppliance.active,
        ...
        // Log EV charging decision
        console.log('EV charging status:', {
          connected: this.energyModels.evConnected,
          currentSoC: this.energyModels.evSoC,
          price: hourData.price,
          chargingPower: evResult.power,
          nextSoC: evResult.nextSoC,
          shouldCharge: evResult.power > 0.1
        });
        ...
        // 1. EV connects and should charge
        if (this.energyModels.evConnected && evResult.power > 0.1 && !evAppliance.active) {
          console.log(`Turning EV charger ON: SoC=${this.energyModels.evSoC.toFixed(2)}, Power=${evResult.power.toFixed(1)}kW`);
          this.toggleApplianceStatus(evAppliance.id);
        } 
        // 2. EV is charging but should stop (full or high price)
        else if (this.energyModels.evConnected && evResult.power < 0.1 && evAppliance.active) {
          console.log(`Turning EV charger OFF: SoC=${this.energyModels.evSoC.toFixed(2)}`);
          this.toggleApplianceStatus(evAppliance.id);
        }
        // 3. EV disconnects
        else if (!this.energyModels.evConnected && evAppliance.active) {
          console.log(`Turning EV charger OFF: EV disconnected`);
          this.toggleApplianceStatus(evAppliance.id);
        }

        this.updateAppliancePower(evAppliance.id, evResult.power);

        // Update appliance power value when active
        if (evAppliance.active) {
          const newPower = parseFloat(evResult.power.toFixed(1));
          evAppliance.power = newPower;
        }
      }
      ...
      // New code: Update Group 1 and Group 3 appliance status based on schedules
      if (this.applianceSchedules) {
        // Calculate current time step from elapsed simulation minutes
        const currentTimeStep = Math.floor(this.simulationElapsedMinutes / 15) % 96;
        
        // Check each appliance schedule
        Object.entries(this.applianceSchedules).forEach(([appId, schedule]) => {
          // Skip Group 2 appliances - they're handled separately
          if (appId === 'hvac' || appId === 'water_heater' || appId === 'ev_charger') {
            return;
          }
          
          // Special case for refrigerator - always stays on
          if (appId === 'refrigerator') {
            const refrigeratorApp = this.simulatorAppliances.find(app => app.type === 'refrigerator');
            if (refrigeratorApp && !refrigeratorApp.active) {
              this.toggleApplianceStatus(refrigeratorApp.id);
            }
            return;
          }
          
          // Find the appliance in simulatorAppliances by type
          const appliance = this.simulatorAppliances.find(app => app.type === appId);
          if (!appliance) return;
          
          // Check if the appliance should be active at this time step
          const shouldBeActive = currentTimeStep >= schedule.startTime && 
                                currentTimeStep < schedule.endTime;
          
          // Only toggle if state needs to change
          if (shouldBeActive !== appliance.active) {
            //console.log(`Scheduling: ${appId} ${shouldBeActive ? 'ON' : 'OFF'} at time ${currentTimeStep} (${formatTimeStep(currentTimeStep)})`);
            this.toggleApplianceStatus(appliance.id);
          }
        });
      }
    ...
    updateSimulationFromHourlyData(hourData) {
      ...
      if (hourData.price) {
        ...
        if (evAppliance && this.energyModels.evConnected) {
          const isPriceLow = hourData.price < 0.15;
          const isPriceHigh = hourData.price > 0.20;
          const evSoC = this.energyModels.evSoC;
          const isFullyCharged = evSoC > 0.8;
          
          // Smart charging logic based on price and SoC
          if (isPriceLow && !isFullyCharged && !evAppliance.active) {
            console.log(`Smart charging: Low price (${hourData.price}), turning EV charger ON`);
            this.toggleApplianceStatus(evAppliance.id);
          } else if (isPriceHigh && evAppliance.active) {
            console.log(`Smart charging: High price (${hourData.price}), turning EV charger OFF`);
            this.toggleApplianceStatus(evAppliance.id);
          }
        }
      }
    ...
    // Update in EnergyFlowDiagram.vue
    generateRlSimulationData() {
      console.log("Requesting RL simulation data...");
      ...
      iframe.onerror = (error) => {
        console.error("Error downloading RL simulation data:", error);
        alert("Failed to generate RL simulation data. Please check the server logs.");
        document.body.removeChild(iframe);
      };
      
      // Add timeout to prevent hanging
      const timeout = setTimeout(() => {
        console.error("Timeout while downloading RL simulation data");
        alert("Request timed out. The server might be processing a large amount of data or experiencing an error.");
      ...
      iframe.onload = () => {
        ...
        console.log("RL simulation data request completed");
        setTimeout(() => {
          if (document.body.contains(iframe)) {
            document.body.removeChild(iframe);
          }
      ...
      // Use fetch instead of iframe for better error handling
      fetch('http://localhost:5000/generate_day_data')
        .then(response => {
          if (!response.ok) {
        ...
        .then(blob => {
          ...
          a.href = url;
          a.download = `rl_simulation_data_${new Date().toISOString().replace(/[:.]/g, '-')}.csv`;
        ...
        .catch(error => {
          console.error("Error fetching RL simulation data:", error);
          alert(`Failed to generate RL simulation data: ${error.message}`);
        })
        .finally(() => {
          clearTimeout(timeout);
          if (document.body.contains(iframe)) {
  ...
  watch: {
    ...
    rlPrediction(newValue) {
      ...
      if (!this.aiScheduleDownloaded) {
        this.generateRlSimulationData();
        this.aiScheduleDownloaded = true
      }
    },
    ...
    gridPower: {
      handler(newValue, oldValue) {
        // Update energy consumption in price chart when grid power changes
        if (newValue !== oldValue && this.isRunning && this.hourlyDataLoaded) {
          const currentStep = this.simulationSteps % 96;
          const currentPrice = this.currentHourData?.price || 0;
          
          // Update the price chart with new grid power value
          this.updatePriceChart(currentStep, currentPrice, newValue);
        }
      }
    },
  },
  ...
  mounted() {
    ...
    this.applianceSchedules = generateApplianceSchedules();
